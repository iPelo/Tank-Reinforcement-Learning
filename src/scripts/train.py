...
def main() -> None:
    ...
    while True:
        ...
        mean_ret10 = float(np.mean(ep_returns[-10:])) if len(ep_returns) >= 10 else (float(np.mean(ep_returns)) if ep_returns else 0.0)
        ...
        if updates % 10 == 0:
            path = os.path.join("models", f"ppo_phase0_u{updates:04d}.pt")
            torch.save({"model": model.state_dict(), "obs_dim": obs_dim, "act_dim": act_dim}, path)
